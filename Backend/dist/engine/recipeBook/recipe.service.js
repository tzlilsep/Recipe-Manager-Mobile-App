...
class RecipeService {
        ...
        `, [userId, input.name, input.prepMinutes, input.totalMinutes, input.servings, input.imageUrl, input.tips]);
            ...
            if (input.labels?.length) {
                for (const label of input.labels) {
                    await client.query('INSERT INTO recipe_labels (recipe_id, label) VALUES ($1, $2)', [recipe.id, label]);
                }
            }
    ...
    async updateRecipe(recipeId, userId, input) {
        const client = await db_1.db.connect();
        try {
            await client.query('BEGIN');
            // Check ownership
            const ownerCheck = await client.query('SELECT owner_user_id FROM recipes WHERE id = $1', [recipeId]);
            ...
            if (updateFields.length > 0) {
                ...
                await client.query(`UPDATE recipes SET ${updateFields.join(', ')} WHERE id = $${paramIndex}`, updateValues);
            }
            // Update labels if provided
            if (input.labels !== undefined) {
                await client.query('DELETE FROM recipe_labels WHERE recipe_id = $1', [recipeId]);
                for (const label of input.labels) {
                    await client.query('INSERT INTO recipe_labels (recipe_id, label) VALUES ($1, $2)', [recipeId, label]);
                }
            }
    ...
    async deleteRecipe(recipeId, userId) {
        const result = await db_1.db.query('DELETE FROM recipes WHERE id = $1 AND owner_user_id = $2 RETURNING id', [recipeId, userId]);
        return result.rows.length > 0;
    }
    // Save (favorite) a recipe
    async saveRecipe(recipeId, userId) {
        try {
            await db_1.db.query('INSERT INTO recipe_saves (recipe_id, user_id) VALUES ($1, $2) ON CONFLICT DO NOTHING', [recipeId, userId]);
            return true;
        }
        catch {
            return false;
    ...
    async unsaveRecipe(recipeId, userId) {
        const result = await db_1.db.query('DELETE FROM recipe_saves WHERE recipe_id = $1 AND user_id = $2 RETURNING recipe_id', [recipeId, userId]);
        return result.rows.length > 0;
    }
    // Helper: Get labels for a recipe
    async getRecipeLabels(recipeId) {
        const result = await db_1.db.query('SELECT label FROM recipe_labels WHERE recipe_id = $1', [recipeId]);
        return result.rows.map(r => r.label);
    }
    // Helper: Get ingredient rows for a recipe
    async getRecipeIngredientRows(recipeId) {
        const result = await db_1.db.query(`
